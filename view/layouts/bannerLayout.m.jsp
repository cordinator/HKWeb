...
<s:layout-definition>
  <c:if test="${pageBanners != null}">
    <c:forEach items="${pageBanners}" var="pageBanner">
      <c:if test="${pageBanner.slotToBanner!=null and fn:length(pageBanner.slotToBanner)>0 and pageBanner.slotToBanner['one']!=null}">
          ...
          <c:forEach items="<%=EnumBannerConstants.getSlots()%>" var="slot">
            ...
            <c:if test="${storePageBanners!=null}">
              <c:if test="${bannerType eq stripBanner}">
                  <s:layout-render name="/layouts/stripBannerLayout.m.jsp" storePageBanners="${storePageBanners}" />
              </c:if>
              <c:if test="${fn:contains(heroBanners, bannerType)}">
                  <s:layout-render name="/layouts/categoryBannerLayout.m.jsp" slot="${slot}" storePageBanners="${storePageBanners}" />
              </c:if>
            </c:if>
          </c:forEach>
      </c:if>
