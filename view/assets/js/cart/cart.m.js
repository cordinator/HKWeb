...
var AddToCart = function() {
  var self = this,
      placeOrderClass = '.addToCart',
      placeOrderPackClass = '.addPackToCart',
      placeOrderWithExtOptClass = '.addToCartWithExtOp',
      addingToCartText = 'Adding to cart',
      goToCartText = 'Go to cart',
      processingFlag = 'processing',
      buyNowClass = 'blue',
      addingToCartClass = 'grey',
      addedToCartClass = "green";

  function setAddingToCartFlag(ref){
    ref.data('status',processingFlag);
    ref.text(processingFlag);
    ref.removeClass(buyNowClass).addClass(addingToCartClass);
  }

  function addedToCart(ref){
    ref.data('status','');
    ref.off('click');
    ref.text(goToCartText);
    ref.removeClass(addingToCartClass).addClass(addedToCartClass);
    ref.attr('href','/beta/cart/Cart.action');
  }

  function addToCartFailed(ref){
    ref.data('status','');
    ref.text('Buy now');
    ref.removeClass(addingToCartClass).addClass(buyNowClass);
  }

  self.init = function() {
    $(placeOrderClass).click(function(event) {
      var ref = $(this);
      if(ref.data('status') === processingFlag){
        return false;
      } else {
        setAddingToCartFlag(ref);
      }
      ...
      self.sendToCartRequest = function(requestUrl,ref,params,scrollToTop){
        var onSuccess = function(json) {
          ...
          if (json.exception) {
            ...
            alert("Product can not be added to the cart, try after some time");
            addToCartFailed(ref);
          } else {
